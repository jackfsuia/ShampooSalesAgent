...
A LLM-powered minimal agent that sells Shampoo (or any product provided a product information) for a living. Fluent conversation with customers on Web and will record their orders in a local .csv [file](customer_orders.csv). Support all models, including openai models, huggingface models, baichuan(百川), qwen(通义千问), moonshot(月之暗面). Very friendly for beginners, researchers and interested businessman who want to try things out quick.
